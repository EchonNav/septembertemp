// EchoNav – Urban obstacle detection + speech (LiDAR)
// SwiftUI + RealityKit + ARKit + AVAudioEngine + AVSpeechSynthesizer
...
final class ProximityViewModel: NSObject, ObservableObject {
    ...
    // Tuning (file-visible so Coordinator can read them)
    ...
    fileprivate var lastLabelSpoken: String = ""
    fileprivate var lastSpeechTime: CFTimeInterval = 0
    fileprivate let minSpeechGap: CFTimeInterval = 2.0 // seconds
    ...
    fileprivate func speak(label: String, distance: Float, level: WarningLevel) {
        // Rate limit + avoid spam on same label/distance
        let now = CACurrentMediaTime()
        guard now - lastSpeechTime > minSpeechGap else { return }

        var phrase: String
        let meters = String(format: "%.1f", distance)
        switch level {
        case .high:   phrase = "Danger, \(label) à \(meters) mètre"
        case .medium: phrase = "Attention, \(label) à \(meters) mètre"
        case .low, .none: phrase = "\(label) à \(meters) mètre"
        }
...
struct ARViewContainer: UIViewRepresentable {
    ...
    final class Coordinator: NSObject, ARSessionDelegate {
        ...
        private let sampleHz: Double = 10 // compute at ~10 Hz to save power
        ...
        func session(_ session: ARSession, didUpdate frame: ARFrame) {
            ...
            // === Multi-ray sampling along a horizontal line ===
            // Goal: coarse classification wall vs obstacle étroit using hit coverage
            let w = arView.bounds.width
            let h = arView.bounds.height
            let y = h * 0.5
            let offsets: [CGFloat] = [-0.25, -0.12, 0.0, 0.12, 0.25] // from center, as fraction of width
            var hits: [Float] = []
            ...
            if closest == .greatestFiniteMagnitude {
                handleNoHit()
                return
            }

            // Heuristic classification
            let label = classify(hits: hits)
            handleDistance(closest, label: label)
        }

        private func classify(hits: [Float]) -> String {
            // Count valid hits and check lateral coverage + variance
            let valid = hits.enumerated().compactMap { (i, d) -> (Int, Float)? in
                guard d.isFinite else { return nil }
                return (i, d)
            }
            if valid.isEmpty { return "aucun obstacle" }

            let distances = valid.map { $0.1 }
            let mean = distances.reduce(0, +) / Float(distances.count)
            let variance = distances.map { ($0 - mean) * ($0 - mean) }.reduce(0, +) / Float(max(1, distances.count - 1))
            let std = sqrt(max(0, variance))

            // Coverage: how many adjacent samples hit
            var longestRun = 0
            var currentRun = 0
            for d in hits {
                if d.isFinite { currentRun += 1; longestRun = max(longestRun, currentRun) }
                else { currentRun = 0 }
            }

            // Wall: wide coverage + low variance
            if longestRun >= 4 && std < 0.25 { return "mur" }
            // Narrow object: few hits with gaps around
            if longestRun <= 2 { return "poteau" }
            return "obstacle"
        }

        private func handleDistance(_ d: Float, label: String) {
            let clamped = max(0, min(d, vm.maxSense))
            vm.distanceText = String(format: "%.2f m", clamped)

            if d < vm.nearThreshold {
                vm.statusText = "DANGER: \(label) très proche"
                ...
                vm.speak(label: label, distance: d, level: .high)
            } else if d < vm.midThreshold {
                vm.statusText = "Attention: \(label) proche"
                ...
                vm.speak(label: label, distance: d, level: .medium)
